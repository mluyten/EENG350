...
void moveRobot(float desiredPosition, float desiredAngle) {
  ...
  while (positionCounter < 10) {
    ...
    if (velocityChanged) { // If encoder counts changes on either wheel, update velocity variables
      ...
      //Serial.print("\t");
      //Serial.print(deltaForward);
      //Serial.print("\t");
      Serial.print(abs(desiredPosition - currentPosition));
      Serial.print("\t");
      Serial.println(abs(desiredAngle - currentAngle));
      if ((abs(deltaPosition) < 0.1) && (abs(deltaAngle) <= 0.02)) {
        if (inFinalPosition) {
          positionCounter++;
